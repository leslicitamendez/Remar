...
def full_name
  "#{self.nombre} #{self.apellido1} #{self.apellido2}"
	...
	validates :direccion, format: {allow_blank: true, with: /\A[a-zA-Z0-9 #\W]+\z/, message: "- Solo se aceptan letras"}

	validates :estadoCivil, presence: {:message => "- El Estado Civil es un campo obligatorio"}

	validates :sexo, presence: {:message => "- El campo sexo es un campo obligatorio"}
	...
	validates :profesion, format: { with: /\A[a-zA-Z \W]+\z/, message: "- Solo se aceptan letras en el campo profesion"}, allow_blank: true
	
	validates :oficio, format: { with: /\A[a-zA-Z ]+\z/, message: "- Solo se aceptan letras en el campo oficio"}, allow_blank: true

	validates :estudios, format: { with: /\A[a-zA-Z \W]+\z/, message: "- Solo se aceptan letras en el campo Estudios"}, allow_blank: true
